...
public class SignRender extends Module {
    ...
    public SignRender() {
        super(HappyAccidents.CATEGORY, "sign-render", "Renders sign text through walls with advanced clustering.");
    }
    @EventHandler
    private void onRender2D(Render2DEvent event) {
        if (mc.world == null || mc.player == null) return;
        currentDrawContext = event.drawContext;
        updateTicker++;
    ...
    private void renderMultilineSign(SignRenderData sign, TextRenderer textRenderer, double centerX, double centerY) {
        ...
        if (showBackground.get()) {
            ...
            if (currentDrawContext != null) {
                currentDrawContext.fill(
                    (int) Math.round(bgLeft),
                    (int) Math.round(bgTop),
                    (int) Math.round(bgLeft + bgWidth),
                    (int) Math.round(bgTop + bgHeight),
                    bgColor.getPacked()
                );
            }
        }
    ...
    private void renderTextAtScreenPos(String text, double screenX, double screenY, double scale, Color color, TextRenderer textRenderer) {
        ...
        if (showBackground.get()) {
            ...
            if (currentDrawContext != null) {
                currentDrawContext.fill(
                    (int) Math.round(elementLeft),
                    (int) Math.round(elementTop),
                    (int) Math.round(elementLeft + elementWidth),
                    (int) Math.round(elementTop + elementHeight),
                    bgColor.getPacked()
                );
            }
        }
